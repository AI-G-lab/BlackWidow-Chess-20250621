...
public final class ImprovedBoard {
    ...
    private volatile Collection<Move> allLegalMoves;
    private volatile Collection<Piece> allPieces;
    
    // Cache for legal moves to avoid recalculation
    private static final Map<String, Collection<Move>> LEGAL_MOVES_CACHE = new ConcurrentHashMap<>();
    private static final int MAX_CACHE_SIZE = 1000;

    private static final ImprovedBoard STANDARD_BOARD = createStandardBoardImpl();

    private ImprovedBoard(final Builder builder) {
        ...
        // Lazy initialization - don't calculate legal moves during construction
        final Collection<Move> whiteStandardMoves = Collections.emptyList();
        final Collection<Move> blackStandardMoves = Collections.emptyList();
    ...
    public Collection<Piece> getAllPieces() {
        if (allPieces == null) {
            synchronized (this) {
                if (allPieces == null) {
                    final List<Piece> pieces = new ArrayList<>(this.whitePieces.length + this.blackPieces.length);
                    for (final int index : this.whitePieces) {
                        pieces.add(this.boardConfig[index]);
                    }
                    for (final int index : this.blackPieces) {
                        pieces.add(this.boardConfig[index]);
                    }
                    allPieces = Collections.unmodifiableList(pieces);
                }
            }
        }
        return allPieces;
    ...
    public Collection<Move> getAllLegalMoves() {
        if (allLegalMoves == null) {
            synchronized (this) {
                if (allLegalMoves == null) {
                    allLegalMoves = Stream.concat(
                        getLegalMovesForPlayer(this.whitePlayer).stream(),
                        getLegalMovesForPlayer(this.blackPlayer).stream()
    ...
    private Collection<Move> getLegalMovesForPlayer(final Player player) {
        final String cacheKey = createCacheKey(player);
        Collection<Move> cachedMoves = LEGAL_MOVES_CACHE.get(cacheKey);
        
        if (cachedMoves != null) {
            return cachedMoves;
        }
        
        final Collection<Move> legalMoves = calculateLegalMoves(this.boardConfig, 
            player.getAlliance().isWhite() ? this.whitePieces : this.blackPieces);
        
        // Cache with size limit
        if (LEGAL_MOVES_CACHE.size() < MAX_CACHE_SIZE) {
            LEGAL_MOVES_CACHE.put(cacheKey, legalMoves);
        }
        
        return legalMoves;
    }
    
    private String createCacheKey(final Player player) {
        // Simple cache key - in production, use zobrist hashing
        return player.getAlliance().toString() + "_" + this.toString().hashCode();
    }
    ...
    public static ImprovedBoard createStandardBoard() {
        return STANDARD_BOARD;
    }

    private static ImprovedBoard createStandardBoardImpl() {
    ...
    public static class Builder {
        ...
        public ImprovedBoard build() {
            return new ImprovedBoard(this);
        }
    }
}
